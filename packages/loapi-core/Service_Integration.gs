 ...
 * @description 
 * 분산된 28개 이상의 워커(Worker) 시트에서 수집된 데이터를 중앙 마스터 DB로 통합하는 ETL 파이프라인.
 * 
 ...
 *    -> 이를 통해 1억 셀 이상의 데이터 환경에서도 1초 이내의 응답 속도 확보
 * 
...
class HybridIntegrator {
  ...
  _processHistory(masterSheet, urls, masterHeaders, startIndex, dataStartRow, dataStartCol) {
    ...
    for (let i = startIndex; i < urls.length; i++) {
      ...
      // [Auto-Save] 파일 단위 처리 성공 시마다 체크포인트 갱신
      // - 트랜잭션 개념: 파일 하나 완료 = 커밋(Commit)
      // - 중간에 실패해도 이미 처리된 파일은 다시 처리하지 않음
      // - 데이터 무결성 보장: 중복 입력 방지
