 ...
 * 3. **Efficiency**: Crypto Cache (SHA-256 키 해싱) 전략을 통해 동일 요청에 대한 중복 호출 원천 차단
 */
...
function _executeRequest(url, params, retryCount = 0) {
  ...
  try {
    ...
    if (code === 200) {
      ...
      // [Rate Limit Handling] 429(Too Many Requests)는 아래 지수 백오프로 대기 후 재시도
...
function _executeWithCache(url, params) {
  // Payload가 길어질 수 있으므로 SHA-256 해시로 고정 길이 키 생성
  const signature = JSON.stringify([url, params.payload || 'GET']);
  const cacheKey = Utilities.base64Encode(Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, signature)); // 해시 키 — 길이 고정·충돌 회피
