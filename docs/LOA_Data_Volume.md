...
Google Sheets를 NoSQL처럼 활용하는 데이터 처리 시스템입니다.
단일 시트 한계(셀 1,000만 개)를 분산 저장(Sharding)과 계층적 통합으로 우회해, 5개 분류·약 1억 셀 규모의 시계열 데이터를 운영합니다.

> 이 문서의 행 수·파일 구성 수치는 2026-01-27 스냅샷 기준입니다. 수집이 계속되고 있어 현재 수치는 이보다 큽니다.

---

## 핵심 지표 (Key Metrics)

- **총 데이터 규모**: **약 1억 셀**(상세 표 합계 ≈112M Cells)
- **일일 수집량**: 분 단위 실시간 수집 (1분/5분 주기)
- **운영 기간**: 2021.12 수집 시작 · 2024.07 서비스 공개 · 현재 운영 중 (데이터 4년+ 누적)
...
| :------------------------------- | :-------------- | :----------- | :--------------------- |
| 강화재료(26.01.01~)      | 112개           | 7,377행      | 🟢 트리거(5분) 작동 중 |
| 각인서(26.01.01~)        | 43개            | 7,380행      | 🟢 트리거(5분) 작동 중 |
| 보석(26.01.01~)          | 12개            | 7,410행      | 🟢 트리거(5분) 작동 중 |
| 생활&항해(26.01.01~)     | 39개            | 7,384행      | 🟢 트리거(5분) 작동 중 |
| 요리&전투용품(26.01.01~) | 76개            | 7,382행      | 🟢 트리거(5분) 작동 중 |
...
| :-------------------------- | :-------------- | :----------- | :--------------------- |
| 강화재료(26.01.01~) | 112개           | 36,095행     | 🟢 트리거(1분) 작동 중 |
| 각인서(26.01.01~)   | 43개            | 75,903행     | 🟢 트리거(1분) 작동 중 |
...
- **효과**: 단일 파일 한계 회피, 안정적 운영

### 2. 비용 효율성 (Cost Efficiency)

- **문제**: 유료 DB 사용 시 비용 발생
- **해결**: 유료 DB 없이 Google Sheets만으로 4년치 시계열 데이터 파이프라인을 운영
- **효과**: 월 비용 0원 유지
...
- **효과**: 새 분기 파일을 추가하면 코드 변경 없이 수집 대상이 늘어남
...
- **2025.01**: 품목 확대 (강화재료 수집 품목 118개까지 증가)
- **2026.01**: 약 1억 셀 규모 운영 중

> 품목 수 집계 기준: 성장 추이의 수치는 각 시점의 **수집 품목 수**, 상단 마스터 표의
> 열 수(강화재료 112개)는 **2026-01-27 스냅샷의 마스터 시트 열 수**다. 게임 업데이트에
> 따라 품목 구성이 바뀌므로 두 수치는 기준 시점·집계 단위가 다르다.

---

## 결론

유료 DB 없이 Google Sheets만으로 4년치·약 1억 셀 규모의 시계열 데이터를 24시간 무중단으로 수집·운영하고, 분산 저장으로 단일 시트 한계를 우회한 구조입니다.
