...
여러 데이터 수집 워커(Worker)가 생성한 분산된 파일의 데이터를 중앙 마스터 시트(Google Sheets)로 통합하는 ETL(Extract, Transform, Load) 프로세스를 담당합니다(체크포인트 재개로 중복·누락 없는 적재를 목표로 설계).
...
- **핵심 변경**: '이어하기' 도입. 작업이 중단되어도 다음 실행 시 정확히 중단된 파일부터 작업을 재개.
