/**
 * @fileoverview [Worker] 실시간 시장 데이터 수집 워커 (v1.0.0, 1분/TPM)
 * @description
 * 중앙 라이브러리(Loapi)를 호출하여 1분 단위로 실시간 데이터를 수집하는
 * 분류별 수집 워커 중 하나입니다 (현재 7개 가동). (Role: 강화재료 수집)
 *
 * [Mechanism]
...
function handleTickUpdate() {
  ...
  try {
    ...
    if (price.length > 1) {
      ...
      // [DB Write] appendRow로 행 단위 원자 기록 (셀 단위 개별 쓰기 대비 호출 1회)
      sheet.appendRow(row);
